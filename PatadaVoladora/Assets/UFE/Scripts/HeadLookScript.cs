...
public class BendingSegment {
	...
	public float maxBendingHorizontal = 80;
	public float maxBendingVertical = 80;
...
public class HeadLookScript : MonoBehaviour {
	...
	public Vector3 headLookVector = Vector3.right;
	...
	void LateUpdate () {
		...
		foreach (BendingSegment segment in segments) {
			...
			hAngle = Mathf.Clamp(hAngle, -segment.maxBendingHorizontal, segment.maxBendingHorizontal);
			vAngle = Mathf.Clamp(vAngle, -segment.maxBendingVertical, segment.maxBendingVertical);
